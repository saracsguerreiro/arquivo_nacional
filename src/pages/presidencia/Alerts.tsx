...
const P = {
  urgente: { color: '#CE1126', bg: '#CE112618', border: '#CE112640', label: 'URGENTE', icon: <AlertTriangle size={14} /> },
  alta: { color: '#D97706', bg: '#D9770618', border: '#D9770640', label: 'ALTA', icon: <AlertCircle size={14} /> },
  normal: { color: '#555555', bg: '#55555518', border: '#55555540', label: 'NORMAL', icon: <Info size={14} /> },
}
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0A0A0A' }}>
      ...
      <div style={{ maxWidth: 720, margin: '0 auto', padding: '28px 20px' }}>
        <button onClick={() => navigate('/presidencia')} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 13, color: '#555555', marginBottom: 24 }}>
          <ArrowLeft size={15} /> Voltar ao resumo
        </button>

        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#FFFFFF', marginBottom: 4 }}>Alertas e Prioridades</h1>
        <p style={{ fontSize: 13, color: '#555555', marginBottom: 28 }}>Destaques do Executivo ordenados por urgência</p>
            ...
            <section key={group.key} style={{ marginBottom: 28 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {group.items.map((alert, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: '#141414', border: `1px solid ${cfg.border}`, borderRadius: 10, overflow: 'hidden' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 16px', background: cfg.bg }}>
                        ...
                        <span style={{ fontSize: 11, color: '#555555' }}>· {alert.ministry}</span>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginLeft: 'auto', fontSize: 11, color: '#444444' }}>
                          <Clock size={10} /> {alert.time}
                        </div>
                      </div>
                      <div style={{ padding: '14px 16px' }}>
                        <p style={{ fontSize: 14, fontWeight: 700, color: '#EEEEEE', marginBottom: 6 }}>{alert.topic}</p>
                        <p style={{ fontSize: 13, color: '#777777', lineHeight: 1.65 }}>{alert.summary}</p>
                        <p style={{ fontSize: 11, color: '#444444', marginTop: 10 }}>{alert.posts} publicações no Espaço do Governo</p>
                      </div>
                    </div>
                  )
                })}
